...
const char* model_path = "model/yolov10s.rknn";  // 模型路径
...
（1）模型结构：[yolov10](https://github.com/THU-MIG/yolov10)

yolo-coco 行人检测调用方法：

```
//coco_person_det, main.cpp 文件中的关键字
const char* model_path = "model/yolov10s.rknn";  // 模型路径
object_detect_result_list result = inference_person_det_model(&rknn_app_ctx, input_data, true); 
```

（2）模型结构：ssd，相比于yolov10,推理速度更快
...
调用方法：参考main.cpp 代码中的face_attr的调用方法;
