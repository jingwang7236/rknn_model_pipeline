...
int inference_yolov8_model(rknn_app_context_t* app_ctx, void* image_buf, object_detect_result_list* od_results, letterbox_t letter_box, int class_num,
	float nms_threshold = NMS_THRESH, float box_conf_threshold = BOX_THRESH, bool enable_logger = true);

int inference_yolov8_obb_model(rknn_app_context_t* app_ctx, image_buffer_t* img, object_detect_obb_result_list* od_results);

int inference_yolov8_obb_model(rknn_app_context_t* app_ctx, void* image_buf, object_detect_obb_result_list* od_results, letterbox_t letter_box,
	float nms_threshold = NMS_THRESH, float box_conf_threshold = BOX_THRESH, bool enable_logger = false);

int inference_yolov8_obb_model(rknn_app_context_t* app_ctx, void* image_buf, object_detect_obb_result_list* od_results, letterbox_t letter_box, int class_num,
