...
/* 定义类别字典 */
std::map<int, std::string> det_stat_door_category_map = {
    {0, "closed"},
    {1, "open"}
};
 ...
 */
 ...
 /*
 object_detect_result_list inference_det_stat_door_model(
...
object_detect_result_list inference_det_stat_door_model(rknn_app_context_t* app_ctx, det_model_input input_data, model_inference_params params_, bool det_by_square, bool enable_logger) {
    ...
    if (enable_logger) {
        ...
        for (int i = 0; i < od_results.count; i++) {
            ...
            printf("%s @ (%d %d %d %d) %.3f\n", det_stat_door_category_map[det_result->cls_id].c_str(),
            ...
            if (draw_box) {
                ...
                sprintf(text, "%s %.1f%%", det_stat_door_category_map[det_result->cls_id].c_str(), det_result->prop * 100);
                draw_text(&src_image, text, x1, y1 - 20, COLOR_RED, 10);
            }
        }

        if (draw_box) {
            const char* image_path = "draw_result_det_door.png";
