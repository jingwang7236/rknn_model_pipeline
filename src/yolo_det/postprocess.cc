...
static int nms_obb(int validCount, std::vector<float>& outputLocations, std::vector<int> classIds, std::vector<int>& order,
    int filterId, float threshold) {
    for (int i = 0; i < validCount; ++i) {
...
static int nms_pose(int validCount, std::vector<float>& outputLocations, std::vector<int> classIds, std::vector<int>& order,
    int filterId, float threshold)
{
    for (int i = 0; i < validCount; ++i)
    {
...
static int process_u8_obb(uint8_t* input, uint8_t* angle_feature, int grid_h, int grid_w, int stride,
...
static int process_u8_pose(uint8_t* input, int grid_h, int grid_w, int stride,
    std::vector<float>& boxes, std::vector<float>& boxScores, std::vector<int>& classId, float threshold,
...
static int process_i8_obb(int8_t* input, int8_t* angle_feature, int grid_h, int grid_w, int stride,
    std::vector<float>& boxes, std::vector<float>& boxScores, std::vector<int>& classId, float threshold,
    int32_t zp, float scale, int32_t angle_feature_zp, float angle_feature_scale, int index) {
    int input_loc_len = 64;
    int tensor_len = input_loc_len + OBJ_CLASS_NUM;
...
static int process_i8_pose(int8_t* input, int grid_h, int grid_w, int stride,
    std::vector<float>& boxes, std::vector<float>& boxScores, std::vector<int>& classId, float threshold,
    int32_t zp, float scale, int index) {
    int input_loc_len = 64;
    int tensor_len = input_loc_len + OBJ_CLASS_NUM;
...
static int process_fp32_obb(float* input, float* angle_feature, int grid_h, int grid_w, int stride,
    std::vector<float>& boxes, std::vector<float>& boxScores, std::vector<int>& classId, float threshold,
    int32_t zp, float scale, int32_t angle_feature_zp, float angle_feature_scale, int index) {
    int input_loc_len = 64;
    int tensor_len = input_loc_len + OBJ_CLASS_NUM;
...
static int process_fp32_pose(float* input, int grid_h, int grid_w, int stride,
    std::vector<float>& boxes, std::vector<float>& boxScores, std::vector<int>& classId, float threshold,
    int32_t zp, float scale, int index) {
    int input_loc_len = 64;
    int tensor_len = input_loc_len + OBJ_CLASS_NUM;
...
static int process_u8_v8(uint8_t* box_tensor, int32_t box_zp, float box_scale,
    uint8_t* score_tensor, int32_t score_zp, float score_scale,
    uint8_t* score_sum_tensor, int32_t score_sum_zp, float score_sum_scale,
    int grid_h, int grid_w, int stride, int dfl_len,
    std::vector<float>& boxes,
...
static int process_i8_v8(int8_t* box_tensor, int32_t box_zp, float box_scale,
    int8_t* score_tensor, int32_t score_zp, float score_scale,
    int8_t* score_sum_tensor, int32_t score_sum_zp, float score_sum_scale,
    int grid_h, int grid_w, int stride, int dfl_len,
    std::vector<float>& boxes,
...
static int process_fp32_v8(float* box_tensor, float* score_tensor, float* score_sum_tensor,
    int grid_h, int grid_w, int stride, int dfl_len,
    std::vector<float>& boxes,
    std::vector<float>& objProbs,
    std::vector<int>& classId,
...
int post_process_det(rknn_app_context_t* app_ctx, void* outputs, letterbox_t* letter_box, float conf_threshold, float nms_threshold, object_detect_result_list* od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (app_ctx->is_quant)
        {
            ...
            validCount += process_i8_v8((int8_t*)_outputs[box_idx].buf, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
        ...
        {
            validCount += process_fp32_v8((float*)_outputs[box_idx].buf, (float*)_outputs[score_idx].buf, (float*)score_sum,
                grid_h, grid_w, stride, dfl_len,
                filterBoxes, objProbs, classId, conf_threshold);
        }
...
int post_process_obb(rknn_app_context_t* app_ctx, void* outputs, letterbox_t* letter_box, float conf_threshold, float nms_threshold, object_detect_obb_result_list* od_results) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        stride = model_in_h / grid_h;
        if (app_ctx->is_quant) {
            validCount += process_i8_obb((int8_t*)_outputs[i].buf, (int8_t*)_outputs[3].buf, grid_h, grid_w, stride, filterBoxes, objProbs,
            ...
            validCount += process_fp32_obb((float*)_outputs[i].buf, (float*)_outputs[3].buf, grid_h, grid_w, stride, filterBoxes, objProbs,
                classId, conf_threshold, app_ctx->output_attrs[i].zp, app_ctx->output_attrs[i].scale,
                app_ctx->output_attrs[3].zp, app_ctx->output_attrs[3].scale, index);
        }
        index += grid_h * grid_w;
    ...
    for (auto c : class_set) {
        nms_obb(validCount, filterBoxes, classId, indexArray, c, nms_threshold);
    }
...
int post_process_pose(rknn_app_context_t* app_ctx, void* outputs, letterbox_t* letter_box, float conf_threshold, float nms_threshold,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        stride = model_in_h / grid_h;
        if (app_ctx->is_quant) {
            validCount += process_i8_pose((int8_t*)_outputs[i].buf, grid_h, grid_w, stride, filterBoxes, objProbs,
                classId, conf_threshold, app_ctx->output_attrs[i].zp, app_ctx->output_attrs[i].scale, index);
        }
        else
        {
            validCount += process_fp32_pose((float*)_outputs[i].buf, grid_h, grid_w, stride, filterBoxes, objProbs,
                classId, conf_threshold, app_ctx->output_attrs[i].zp, app_ctx->output_attrs[i].scale, index);
        }
        index += grid_h * grid_w;
    }
    ...
    for (auto c : class_set) {
        nms_pose(validCount, filterBoxes, classId, indexArray, c, nms_threshold);
    }
