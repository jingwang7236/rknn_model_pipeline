...
object_detect_result_list inference_det_hand_model(rknn_app_context_t *app_ctx, det_model_input input_data, model_inference_params params_, bool enable_logger) {
    object_detect_result_list od_results;
    int ret = 0;

    ImagePreProcess det_hand_image_preprocess(input_data.width, input_data.height, params_.input_width, params_.input_height);
    auto convert_img = det_hand_image_preprocess.Convert(convertDataToCvMat(input_data));
    // cv::Mat img_rgb = cv::Mat::zeros(INPUT_WIDTH_DET_HAND, INPUT_HEIGHT_DET_HAND, convert_img->type());
    // convert_img->copyTo(img_rgb);
    ret = inference_yolov8_model(app_ctx, convert_img->ptr<unsigned char>(), &od_results, det_hand_image_preprocess.get_letter_box(), 
                                det_hand_category_map.size(), params_.nms_threshold, params_.box_threshold, enable_logger);
