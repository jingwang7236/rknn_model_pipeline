...
// 定义类别字典
std::map<int, std::string> det_kx_category_map = {
    {0, "kx"},
    {1, "kx_dk"}
};
...
//     }
//     ImagePreProcess det_kx_image_preprocess(input_data.width, input_data.height, INPUT_WIDTH_DET_KX, INPUT_HEIGHT_DET_KX);
...
//             printf("%s @ (%d %d %d %d) %.3f\n", det_kx_category_map[det_result->cls_id].c_str(),
...
//                 sprintf(text, "%s %.1f%%", det_kx_category_map[det_result->cls_id].c_str(), det_result->prop * 100);
...
object_detect_result_list inference_det_kx_model(rknn_app_context_t *app_ctx, det_model_input input_data, model_inference_params params_, bool enable_logger) {
    object_detect_result_list od_results;
    int ret = 0;

    ImagePreProcess det_kx_image_preprocess(input_data.width, input_data.height, params_.input_width, params_.input_height);
    auto convert_img = det_kx_image_preprocess.Convert(convertDataToCvMat(input_data));
    // cv::Mat img_rgb = cv::Mat::zeros(INPUT_WIDTH_DET_KX, INPUT_HEIGHT_DET_KX, convert_img->type());
    // convert_img->copyTo(img_rgb);
    ret = inference_yolov8_model(app_ctx, convert_img->ptr<unsigned char>(), &od_results, det_kx_image_preprocess.get_letter_box(), 
                                det_kx_category_map.size(), params_.nms_threshold, params_.box_threshold, enable_logger);
