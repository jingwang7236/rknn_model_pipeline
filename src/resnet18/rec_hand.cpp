...
// 定义类别字典
std::map<int, std::string> rec_hand_category_map = {
    {0, "gp"},
    {1, "qm"},
    {2, "wsj"},
    {3, "gp_sz"},
...
resnet_result inference_rec_hand_resnet18_model(rknn_app_context_t *app_ctx, det_model_input input_data, bool enable_logger)
{
    ...
    // 分配内存用于存储调整大小后的图像
    unsigned char* resized_data = (unsigned char*)malloc(IMG_WIDTH_REC_HAND * IMG_HEIGHT_REC_HAND * input_data.channel);
    ...
    if (!stbir_resize_uint8_linear(input_data.data, input_data.width, input_data.height, 0,
                                   resized_data, IMG_WIDTH_REC_HAND, IMG_HEIGHT_REC_HAND, 0, READ_IMAGE_TYPE_REC_HAND)) {
    ...
    */
    ...
    src_image.width = IMG_WIDTH_REC_HAND;
    src_image.height = IMG_HEIGHT_REC_HAND;
    src_image.format = IMAGE_FORMAT_RGB888;
    src_image.size = IMG_WIDTH_REC_HAND * IMG_HEIGHT_REC_HAND * input_data.channel;
    // src_image.virt_addr = resized_img.data;
    src_image.virt_addr = resized_data;
    
    int ret = inference_resnet_model(app_ctx, &src_image, &od_results, REC_HAND_RET_TOP_K);
    if (ret != 0)
    {
        od_results.cls = -1;
        od_results.score = -1;
        printf("ERROR: init_rec_hand_resnet_model fail! ret=%d\n", ret);
        free(resized_data);
        return od_results;
    }
...
resnet_result inference_rec_hand_resnet18_model(rknn_app_context_t *app_ctx, det_model_input input_data, cls_model_inference_params params_, bool enable_logger)
{
    ...
    // 调整图像大小
    if (!stbir_resize_uint8_linear(input_data.data, input_data.width, input_data.height, 0,
                                   resized_data, params_.img_width, params_.img_height, 0, READ_IMAGE_TYPE_REC_HAND)) {
        printf("ERROR: Failed to resize image\n");
        free(resized_data);
        od_results.cls = -2;
        od_results.score = -2;
