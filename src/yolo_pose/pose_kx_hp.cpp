...
/*-------------------------------------------
                Defines
-------------------------------------------*/
#define INPUT_WIDTH_POSE_KX_HP 480
#define INPUT_HEIGHT_POSE_KX_HP 480
#define CLASS_NUM_POSE_KX_HP 1
#define KPT_NUM_POSE_KX_HP 6
#define RESULT_NUM_POSE_KX_HP 4725
#define NMS_THRESH_POSE_KX_HP 0.5
#define BOX_THRESH_POSE_KX_HP 0.25

// 定义类别字典
std::map<int, std::string> pose_kx_hp_category_map = {
    {0, "kx_hp"}
};
 ...
 */
object_detect_pose_result_list inference_pose_kx_hp_model(rknn_app_context_t *app_ctx, det_model_input input_data, bool enable_logger) {
    object_detect_pose_result_list od_results;
    int ret = 0;

    ImagePreProcess pose_kx_hp_image_preprocess(input_data.width, input_data.height, INPUT_WIDTH_POSE_KX_HP, INPUT_HEIGHT_POSE_KX_HP);
    auto convert_img = pose_kx_hp_image_preprocess.Convert(convertDataToCvMat(input_data));

    ret = inference_yolov8_pose_mdoel(app_ctx, convert_img->ptr<unsigned char>(), &od_results, pose_kx_hp_image_preprocess.get_letter_box(),
                                    CLASS_NUM_POSE_KX_HP, KPT_NUM_POSE_KX_HP, RESULT_NUM_POSE_KX_HP,
                                    NMS_THRESH_POSE_KX_HP, BOX_THRESH_POSE_KX_HP, enable_logger);
    ...
    if (enable_logger){
        ...
        for (int i = 0; i < od_results.count; i++) {
            ...
            printf("%s @ (%d %d %d %d) %.3f\n", pose_kx_hp_category_map[det_result->cls_id].c_str(),
            ...
            if (draw_box) {
                ...
                sprintf(text, "%s %.1f%%", pose_kx_hp_category_map[det_result->cls_id].c_str(), det_result->prop * 100);
