...
/*-------------------------------------------
                  Functions
-------------------------------------------*/
int inference_phone_det_model(cv::Mat orig_img, retinaface_result* result)
{
  // cv::Mat orig_img_rgb;
  ...
  if (modelSwitch) {
      ...
      for (int i = 0; i < result->count; ++i) {
          if (enable_draw_image) {
              ...
              cv::rectangle(orig_img, box, color, 2);
              cv::Point textOrg(box.x, box.y - 10);
              cv::putText(orig_img, text, textOrg, cv::FONT_HERSHEY_SIMPLEX, 0.9, color, 2);
              
          }
          if (result->object[i].score < phone_det_threshold) {
              continue;
          }
