...
int crop_image(image_buffer_t* src_image, image_buffer_t* dst_image, image_rect_t* large_box){
    int PIXEL_SIZE = 3;  //默认三通道
    int dst_size;
    if (src_image == NULL || large_box == NULL) {
        return -1; // 参数错误
    }
    // 设置目标图像的尺寸
    dst_image->width = large_box->right - large_box->left;
    dst_image->height = large_box->bottom - large_box->top;
    dst_image->format = IMAGE_FORMAT_RGB888;
    // dst_image->format = src_image->format;
    dst_size = dst_image->width * dst_image->height * PIXEL_SIZE;
    // dst_image->virt_addr = (uint8_t *)malloc(dst_size);
    dst_image->size = dst_size;
    dst_image->virt_addr = (unsigned char*)malloc(dst_size);
    
    // 计算源图像中剪裁区域的内存起始位置  
    const unsigned char* src_start = src_image-> virt_addr + (large_box->top * src_image->width + large_box->left) * PIXEL_SIZE;  
    int src_stride = src_image->width * PIXEL_SIZE; // 源图像每行的字节数  
    int dst_stride = dst_image->width * PIXEL_SIZE;     // 目标图像每行的字节数 
    // 逐行复制像素数据 
    for (int y = 0; y < dst_image->height; y++) {
        memcpy(dst_image->virt_addr + y * dst_stride,
