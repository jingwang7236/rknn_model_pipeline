...
static int convert_image_opencv(rknn_app_context_t *app_ctx, const cv::Mat& orig_img, cv::Mat& dist_img)
{   
    // 获取原始图像的宽度和高度
...
int inference_ppocr_det_model(rknn_app_context_t* app_ctx, image_buffer_t* src_img, ppocr_det_postprocess_params* params, ppocr_det_result* out_result)
{
    ...
    cv::Mat cv_img(src_img->height, src_img->width, CV_8UC3, src_img->virt_addr);
    cv::Mat dist_img;
    ret = convert_image_opencv(app_ctx, cv_img, dist_img);
    if (ret < 0) {
    ...
    float scale_w = (float)src_img->width / (float)dist_img.cols;
    float scale_h = (float)src_img->height / (float)dist_img.rows;
...
int inference_ppocr_system_model_new(ppocr_system_app_context* sys_app_ctx, image_buffer_t* src_img, ppocr_det_postprocess_params* params, ppocr_text_recog_array_result_t* out_result)
{
    // TODO：增加检测框的方向转换，将图像上下翻转，分别识别文字，选择分数最高的文字作为识别结果
    int ret;
    // Detect Text
    ppocr_det_result det_results;
    ret = inference_ppocr_det_model(&sys_app_ctx->det_context, src_img, params, &det_results);
    ...
    for (int i=0; i < boxes_result.size(); i++) {
        ...
        if (text_result.score < TEXT_SCORE) {
            // continue;
            //将crop_image图像上下翻转，再次识别文字，作为识别结果
            cv::Mat crop_image_flip;
            cv::rotate(crop_image, crop_image_flip, cv::ROTATE_180);
            // 保存图片crop_image_flip
            // cv::imwrite("crop_image.jpg", crop_image);
            // cv::imwrite("crop_image_flip.jpg", crop_image_flip);
            image_buffer_t text_img_flip;
            memset(&text_img_flip, 0, sizeof(image_buffer_t));
            text_img_flip.width = crop_image_flip.cols;
            text_img_flip.height = crop_image_flip.rows;
            text_img_flip.format = IMAGE_FORMAT_RGB888;
            text_img_flip.size = get_image_size(&text_img_flip);
            text_img_flip.virt_addr = (unsigned char*)malloc(text_img_flip.size);
            if (text_img_flip.virt_addr == NULL) {
                printf("malloc buffer size:%d fail!\n", text_img_flip.size);
                return -1;
            }
            memcpy((void *)text_img_flip.virt_addr, crop_image_flip.data, text_img_flip.size);
            // ppocr_rec_result text_result;
            // text_result.score = 1.0;
            ret = inference_ppocr_rec_model(&sys_app_ctx->rec_context, &text_img_flip, &text_result);
            // printf("text_result: %s %f\n", text_result.str, text_result.score);
            if (ret != 0) {
                printf("inference_ppocr_rec_model fail! ret=%d\n", ret);
                return -1;
            }
            if (text_img_flip.virt_addr != NULL) {
                free(text_img_flip.virt_addr);
            }
        }
        out_result->text_result[out_result->count].box.left_top.x = boxes_result[i][0];
        out_result->text_result[out_result->count].box.left_top.y = boxes_result[i][1];
